...
En local: `npm ci`, configura `.env` y ejecuta `npm run dev`. En producción Node: `npm ci`, `npm run build`, `npm start`. Con Docker: `docker build -t jamrelay .` y `docker compose up -d --build`.
