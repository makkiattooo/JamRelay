...
Lokal: `npm ci`, `.env` konfigurieren, `npm run dev`. Für Produktion: `npm ci`, `npm run build`, `npm start`. Für Docker: `docker build -t tunelink .` und `docker compose up -d --build`.
...
Der aktuelle Compose-Beispielservice heißt `tunelink`, verwendet einen Healthcheck, `cap_drop: ALL` und `no-new-privileges`.
