...
description: Exécuter JamRelay gratuitement ou avec un très petit budget mensuel.
---

# Déploiement gratuit ou économique

JamRelay consomme peu de ressources.
...
```text
Client IA → HTTPS → Cloudflare Tunnel → JamRelay → Spotify
```
...
Une instance privée se contente généralement d’un vCPU partagé, de 512 Mo à 1 Go de RAM et de quelques Go de stockage persistant. Un serveur plus puissant est rarement nécessaire pour JamRelay seul.
