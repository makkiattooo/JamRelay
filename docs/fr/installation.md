...
En local: `npm ci`, configuration de `.env`, puis `npm run dev`. En production Node: `npm ci`, `npm run build`, `npm start`. Avec Docker: `docker build -t jamrelay .` puis `docker compose up -d --build`.
